...
class Board:
    ...
    def __init__(self, board_str):
        ...
        self.grid = [[self.OFF_BOARD for _ in range(self.width)] for _ in range(self.height)]
        self.location = None
    ...
    def rotate(self, turn_dir):
        ...
        if turn_dir == 'R':
            self.grid[row][col] = self.DIRECTIONS[(dir_i + 1) % len(self.DIRECTIONS)]
        else:
    ...
    def next_location(self):
        ...
        if direction == self.RIGHT:
            ...
            next_col = cur_col - 1
            if next_col < 0 or self.grid[cur_row][next_col] == self.OFF_BOARD:
                next_col = self.last_row_space(cur_row)
            if self.grid[cur_row][next_col] == self.WALL:
                return self.location
            else:
                return (cur_row, next_col)
            ...
            next_row = cur_row - 1
            if next_row < 0 or self.grid[next_row][cur_col] == self.OFF_BOARD:
                next_row = self.last_col_space(cur_col)
            if self.grid[next_row][cur_col] == self.WALL:
                return self.location
            else:
                return (next_row, cur_col)
    ...
    def move(self, spaces):
        for _ in range(spaces):
            (cur_row, cur_col) = self.location
            (next_row, next_col) = self.next_location()
            self.grid[next_row][next_col] = self.grid[cur_row][cur_col]
            self.location = (next_row, next_col)
            
    def execute_instructions(self, instructions):
        i = 0
        # TODO: Restructure this and/or move the string parsing functionality
        # into a stand-alone function
        while i < len(instructions):
            instruction = ''
            while i < len(instructions) and instructions[i].isnumeric():
...
def part_1(input_str):
    board_str, instruction_str = input_str.split('\n\n')
    board = Board(board_str)
    board.execute_instructions(instruction_str)
    # print(board)
    row, col = board.location
    facing = board.grid[row][col]
    return 1000 * (row+1) + 4 * (col + 1) + Board.DIRECTIONS.index(facing)
